...
export class AppStateService {
	...
	private _axis: WritableSignal<'alpha' | 'beta'> = signal('alpha');
	...
	public axis: Signal<'alpha' | 'beta'> = this._axis.asReadonly();
	...
	constructor() {
		...
		// Try reading axis from local storage
		try {
			const fromStorage = localStorage.getItem('protractor-axis') || 'alpha';
			if (fromStorage === 'alpha' || fromStorage === 'beta') {
				this._axis.set(fromStorage);
			}
		} catch (e) {}
	}
	...
	setAxis(value: 'alpha' | 'beta') {
		this._axis.set(value);
		localStorage.setItem('protractor-axis', value);
	}
}
